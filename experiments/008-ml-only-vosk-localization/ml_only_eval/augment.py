...
def decode_wav(path: Path) -> np.ndarray:
    ...
    return (signal * (0.82 / peak) if peak else signal).astype(np.float32)
...
def _mix_at_snr(signal: np.ndarray, interference: np.ndarray, snr_db: float) -> np.ndarray:
    signal_rms = _rms(signal)
    interference_rms = _rms(interference)
    if signal_rms == 0 or interference_rms == 0:
        return signal.copy()
    scale = signal_rms / (interference_rms * 10 ** (snr_db / 20))
...
def _pink_noise(length: int, rng: np.random.Generator) -> np.ndarray:
    spectrum = np.fft.rfft(rng.normal(0.0, 1.0, length))
    frequencies = np.fft.rfftfreq(length, d=1 / SAMPLE_RATE_HZ)
    scale = np.ones_like(frequencies)
    scale[1:] = 1 / np.sqrt(frequencies[1:])
    result = np.fft.irfft(spectrum * scale, n=length)
    return (result - np.mean(result)).astype(np.float32)


def _brown_noise(length: int, rng: np.random.Generator) -> np.ndarray:
    result = np.cumsum(rng.normal(0.0, 1.0, length))
    result -= np.mean(result)
    deviation = float(np.std(result))
    return (result / deviation if deviation else result).astype(np.float32)


def _fit_donor(donor: np.ndarray, length: int, rng: np.random.Generator) -> np.ndarray:
...
def _phone_band(signal: np.ndarray) -> np.ndarray:
    ...
    response[frequencies < 220] = 0
    response[frequencies > 3_600] = 0
    low_ramp = (frequencies >= 220) & (frequencies < 350)
    high_ramp = (frequencies > 3_200) & (frequencies <= 3_600)
    response[low_ramp] = (frequencies[low_ramp] - 220) / 130
    response[high_ramp] = (3_600 - frequencies[high_ramp]) / 400
    filtered = np.fft.irfft(spectrum * response, n=signal.size)
    return (np.round(np.clip(filtered, -1, 1) * 127) / 127).astype(np.float32)


def _resample_pitch(signal: np.ndarray, factor: float) -> np.ndarray:
    output_length = max(1, round(signal.size / factor))
    return np.interp(
        np.linspace(0, signal.size - 1, output_length),
...
def augment(
    ...
    if name == "clean":
        ...
        if donor is None:
            raise ValueError("cafe_crosstalk requires a donor")
        result = _mix_at_snr(signal, _fit_donor(donor, signal.size, rng), 6.0)
...
def pcm16_bytes(signal: np.ndarray) -> bytes:
    return np.round(np.clip(signal, -1.0, 1.0) * 32767).astype("<i2").tobytes()
